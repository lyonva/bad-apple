...
def make_plots(file):
    ...
    df["Episode Reward"] = (1 - 0.9 * (27 / 1024)) - df["Episode Reward"]
    # df["Episode Reward"] = 1-0.9*(((1024/0.9)*(1 - df["Episode Reward"]) - 27) / 1024)
    ...
    g = sns.lineplot(df, x="iterations", y="Episode Reward", hue="im", hue_order=im_name, palette='CMRmap')
    plt.show()

    # for i, sub_plot in enumerate(sub_plots):
    #     sub_df = df[ np.isin(df["im"], sub_plot) ]
    #     g = sns.FacetGrid(sub_df, row="map", col="metric", col_order=atts_name, sharex=False, sharey=False,
